import { GridOrientation, GridStrategy, GridScrollDirection, } from "../types/grid";
export function calculateGridPosition(index, dimensions, orientation) {
    "worklet";
...
export function calculateIndexFromRowColumn(row, column, dimensions, orientation) {
    "worklet";
...
export function listToGridObject(list, dimensions, orientation) {
    const positions = {};
...
export function getGridCellFromCoordinates(x, y, dimensions, orientation, totalItems) {
    "worklet";
    const { columns = 3, rows = 3, itemWidth, itemHeight, rowGap = 0, columnGap = 0, } = dimensions;
...
export function reorderGridInsert(positions, activeId, targetIndex, dimensions, orientation) {
    "worklet";
    const newPositions = {};
...
export function reorderGridSwap(positions, activeId, targetId, dimensions, orientation) {
    "worklet";
...
export function setGridPosition(x, y, scrollX, scrollY, itemsCount, positions, id, dimensions, orientation, strategy) {
    "worklet";
    const adjustedX = x + scrollX;
    const adjustedY = y + scrollY;
    const targetCell = getGridCellFromCoordinates(adjustedX, adjustedY, dimensions, orientation, itemsCount);
    ...
    if (strategy === GridStrategy.Insert) {
        positions.value = reorderGridInsert(positions.value, id, targetCell.index, dimensions, orientation);
    }
    else if (strategy === GridStrategy.Swap && targetId) {
        positions.value = reorderGridSwap(positions.value, id, targetId, dimensions, orientation);
    }
}
export function calculateGridContentDimensions(itemsCount, dimensions, orientation) {
    "worklet";
    const { columns = 3, rows = 3, itemWidth, itemHeight, rowGap = 0, columnGap = 0, } = dimensions;
    if (orientation === GridOrientation.Vertical) {
...
export function setGridAutoScroll(x, y, scrollX, scrollY, containerWidth, containerHeight, scrollThreshold, autoScrollDirection) {
    "worklet";
...
export function findItemIdAtIndex(positions, index) {
    "worklet";
    for (const id in positions) {
