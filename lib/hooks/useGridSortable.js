...
import { GridScrollDirection, GridOrientation, GridStrategy, } from "../types/grid";
import { setGridPosition, setGridAutoScroll, calculateGridContentDimensions, } from "../utils/gridCalculations";
export function useGridSortable(options) {
    const { id, positions, scrollY, scrollX, autoScrollDirection, itemsCount, dimensions, orientation, strategy = GridStrategy.Insert, containerWidth = 500, containerHeight = 500, activationDelay, onMove, onDragStart, onDrop, onDragging, isBeingRemoved = false, } = options;
    ...
    useAnimatedReaction(() => ({ x: positionX.value, y: positionY.value }), (current, previous) => {
        var _a;
        if (!movingSV.value) {
            return;
        }
        ...
        const clampedColumn = Math.min(Math.max(0, Math.round(current.x / (itemWidth + columnGap))), (orientation === GridOrientation.Vertical ? columns : Infinity) - 1);
        ...
        if (orientation === GridOrientation.Vertical) {
            ...
            const rows = (_a = dimensions.rows) !== null && _a !== void 0 ? _a : 3;
        ...
        if (onDragging) {
            ...
            if (now - onDraggingLastCallTimestamp.value > THROTTLE_INTERVAL) {
                scheduleOnRN(onDragging, id, newOverItemId, Math.round(current.x), Math.round(current.y));
                onDraggingLastCallTimestamp.value = now;
            }
        }
        topValue.value = current.y;
        leftValue.value = current.x;
        setGridPosition(current.x, current.y, scrollX.value, scrollY.value, itemsCount, positions, id, dimensions, orientation, strategy);
        setGridAutoScroll(current.x, current.y, scrollX.value, scrollY.value, calculatedContainerWidth, calculatedContainerHeight, dimensions.itemHeight, autoScrollDirection);
    }, [
        movingSV,
        dimensions,
        itemsCount,
    ...
    useAnimatedReaction(() => autoScrollDirection.value, (scrollDirection, previousValue) => {
        if (scrollDirection !== null &&
            ...
            const { width: contentWidth, height: contentHeight } = calculateGridContentDimensions(itemsCount, dimensions, orientation);
            ...
            const hasUp = scrollDirection === GridScrollDirection.Up ||
                scrollDirection === GridScrollDirection.UpLeft ||
                scrollDirection === GridScrollDirection.UpRight;
            const hasDown = scrollDirection === GridScrollDirection.Down ||
                scrollDirection === GridScrollDirection.DownLeft ||
                scrollDirection === GridScrollDirection.DownRight;
            const hasLeft = scrollDirection === GridScrollDirection.Left ||
                scrollDirection === GridScrollDirection.UpLeft ||
                scrollDirection === GridScrollDirection.DownLeft;
            const hasRight = scrollDirection === GridScrollDirection.Right ||
                scrollDirection === GridScrollDirection.UpRight ||
                scrollDirection === GridScrollDirection.DownRight;
            if (hasUp) {
    ...
    useAnimatedReaction(() => ({ y: targetScrollY.value, x: targetScrollX.value }), (target, previous) => {
        if (previous === null)
            return;
        if (movingSV.value) {
    ...
    const createPanGesture = () => Gesture.Pan()
        .activateAfterLongPress(activationDelay !== null && activationDelay !== void 0 ? activationDelay : 200)
