...
export const useDroppable = (options) => {
    ...
    const registerWithMeasurement = useCallback((pageX, pageY, width, height) => {
        register(id, {
    ...
    const updateDroppablePosition = useCallback(() => {
        scheduleOnUI(() => {
            ...
            if (measurement.width > 0 && measurement.height > 0) {
                scheduleOnRN(registerWithMeasurement, measurement.pageX, measurement.pageY, measurement.width, measurement.height);
            }
        });
