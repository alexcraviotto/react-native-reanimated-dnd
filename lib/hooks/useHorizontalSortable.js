...
export function useHorizontalSortable(options) {
    const { id, positions, leftBound, autoScrollDirection, itemsCount, itemWidth, gap = 0, paddingHorizontal = 0, containerWidth = 500, onMove, onDragStart, onDrop, onDragging, } = options;
    ...
    const createPanGesture = () => Gesture.Pan()
        ...
        .onStart((event) => {
        "worklet";
        initialItemContentX.value = getItemXPosition(positions.value[id], itemWidth, gap, paddingHorizontal);
